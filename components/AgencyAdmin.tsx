...
const AgencyAdmin: React.FC = () => {
  ...
  const handleAdd = () => {
    ...
    const account: SubAccount = {
       ...
       plan: newAccount.plan as any,
       status: 'active',
       leadValue: newAccount.leadValue || 100,
       twilio: { isVerified: false }
    };
    setSubAccounts([...subAccounts, account]);
    setShowAddModal(false);
    notify(`Account Created: ${account.name}`);
  };
    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-20">
      ...
      <NexusModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add Account" subtitle="Set up a new sub-account">
         <div className="space-y-6">
            ...
            <div className="pt-6">
               <NexusButton className="w-full" size="xl" onClick={handleAdd}>Save Account</NexusButton>
            </div>
         </div>
      </NexusModal>
    </div>
