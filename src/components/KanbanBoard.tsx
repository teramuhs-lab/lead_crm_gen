...
import { Plus, Clock, TrendingUp, Sparkles, BrainCircuit, Loader2, Target, DollarSign, ChevronRight, AlertTriangle } from 'lucide-react';
import { useNexus } from '../context/NexusContext';
import { useAIQueue } from '../context/AIActionQueueContext';
import { api } from '../lib/api';
import { Contact, AISuggestion } from '../types';
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ onContactClick }) => {
  const { contacts, messages, appointments, updateContact, addContact, activeSubAccount, notify } = useNexus();
  const { addProposal } = useAIQueue();
  const [isPrioritizing, setIsPrioritizing] = useState(false);
  const [riskMap, setRiskMap] = useState<Record<string, 'high' | 'medium' | 'low'>>({});
  ...
  const handleAiPrioritize = async () => {
    ...
    try {
      const result = await api.post<{ suggestions: AISuggestion[] }>('/ai/suggestions', {
      ...
      const newRiskMap: Record<string, 'high' | 'medium' | 'low'> = {};
      (result.suggestions || []).forEach(s => {
        if (s.type === 'at_risk' && s.contactId) {
          newRiskMap[s.contactId] = s.priority === 'high' ? 'high' : 'medium';
        }
        addProposal({
          type: 'send_message',
          title: s.title,
          ...
          contactName: contacts.find(c => c.id === s.contactId)?.name,
          payload: { purpose: s.description, channel: 'email' },
        });
      });
      setRiskMap(newRiskMap);
      notify('AI analysis complete. Suggestions added to queue.');
    } catch {
  ...
  return (
    <div className="h-full flex flex-col space-y-4 pb-6">
      <NexusHeader title="Pipeline" subtitle="Manage your sales pipeline and deals">
        ...
        <NexusButton onClick={() => { const name = prompt("Contact name:")?.trim(); if (name) addContact({ name, status: 'Lead' }); }} icon={Plus}>Add Deal</NexusButton>
      </NexusHeader>
              ...
              <div key={col.id} className="w-72 shrink-0 flex flex-col" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, col.id as Contact['status'])}>
                ...
                <div className="flex-1 bg-slate-50 rounded-xl p-3 border border-slate-200/50 space-y-3 thin-scrollbar overflow-y-auto min-h-[400px]">
                  {contacts.filter(c => c.status === col.id && !c.isArchived).map((lead) => (
                    <NexusCard key={lead.id} padding="none" className="bg-white p-4 cursor-grab active:cursor-grabbing hover:border-brand/30 hover:shadow-md group" draggable onDragStart={(e) => handleDragStart(e, lead.id)} onDragEnd={handleDragEnd} onClick={() => onContactClick?.(lead.id)}>
                      ...
                      {riskMap[lead.id] && (
                        <div className="absolute top-3 left-3 flex items-center gap-1">
                          <AlertTriangle className={`w-3 h-3 ${riskMap[lead.id] === 'high' ? 'text-rose-500' : 'text-amber-500'}`} />
                          <span className={`text-[10px] font-bold ${riskMap[lead.id] === 'high' ? 'text-rose-500' : 'text-amber-500'}`}>{riskMap[lead.id]}</span>
                        </div>
                      )}
                  ...
                  <button onClick={() => { const name = prompt("Contact name:")?.trim(); if (name) addContact({ name, status: col.id as any }); }} className="w-full py-6 flex items-center justify-center gap-2 text-slate-400 hover:text-brand hover:bg-white rounded-xl border-2 border-dashed border-slate-200 hover:border-brand/30 transition-all">
