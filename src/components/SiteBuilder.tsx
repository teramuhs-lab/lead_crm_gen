...
const SiteBuilder: React.FC = () => {
  ...
  const handleAddPage = () => {
    ...
    setEditingFunnel({
      ...editingFunnel,
      pages: [...editingFunnel.pages, newPage],
    });
    setActivePageId(newPage.id);
  };

  const handleDeletePage = (pageId: string) => {
    if (!editingFunnel || editingFunnel.pages.length <= 1) return;
    const updatedPages = editingFunnel.pages.filter(p => p.id !== pageId);
    setEditingFunnel({
      ...editingFunnel,
      pages: updatedPages,
    });
  ...
  const handlePreview = async () => {
    ...
    try {
      const url = activePageId
        ? `/funnels/${activeFunnelId}/preview?pageId=${activePageId}`
        : `/funnels/${activeFunnelId}/preview`;
      const res = await api.get<{ data: { html: string } }>(url);
      ...
      <div className="space-y-5 animate-in fade-in duration-500 pb-20">
        ...
        <NexusCard padding="none" className="border-b-[12px] border-brand/5">
           <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              ...
              <div className="w-64">
                 <NexusInput placeholder="Search sites..." icon={Search} className="py-2.5 px-4" />
              </div>
           </div>
    ...
    <div className="fixed inset-0 z-[150] bg-slate-50 flex flex-col animate-in fade-in duration-300">
      <div className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-6">
           ...
           <NexusButton variant="ghost" icon={Eye} onClick={handlePreview}>Preview</NexusButton>
        ...
        <div className="w-80 flex flex-col gap-6 shrink-0 p-6 bg-white border-r border-slate-200">
           <NexusCard padding="none" className="flex-1 flex flex-col overflow-hidden border border-slate-100">
              <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                 ...
                 <button onClick={handleAddPage} className="p-2 text-brand hover:bg-indigo-50 rounded-xl transition-all"><Plus className="w-4 h-4" /></button>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2 thin-scrollbar">
                 {editingFunnel?.pages.map((p, i) => (
                    <div key={p.id} className="relative group/page">
                      ...
                      {editingFunnel.pages.length > 1 && (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleDeletePage(p.id); }}
                          className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-lg text-slate-300 hover:text-rose-500 hover:bg-rose-50 opacity-0 group-hover/page:opacity-100 transition-all z-10"
                        >
        ...
        <div className="flex-1 bg-slate-100/50 overflow-hidden relative flex flex-col p-6">
              ...
              <div className={`mx-auto bg-white shadow-[0_40px_120px_rgba(0,0,0,0.15)] transition-all duration-700 overflow-hidden min-h-full rounded-xl border border-slate-200 ${viewMode === 'desktop' ? 'w-full max-w-5xl' : 'w-[400px]'}`}>
                 ...
                 {activePage?.blocks.map(block => (
                   <div key={block.id} className="relative group border-b border-transparent hover:border-brand/20 transition-all animate-in slide-in-from-bottom-2">
                      ...
                      {block.type === 'features' && (
                        <div className="py-20 px-20 space-y-8 bg-slate-50/50">
                           <input
                             className="w-full text-3xl font-semibold text-slate-900 text-center bg-transparent border-none focus:ring-0 p-0"
                             value={block.title}
                             onChange={(e) => updateBlock(block.id, { title: e.target.value })}
                           />
                           <textarea
                             className="w-full text-sm font-medium text-slate-400 text-center bg-transparent border-none focus:ring-0 resize-none max-w-xl mx-auto"
                             rows={2}
                             value={block.subtitle}
                             onChange={(e) => updateBlock(block.id, { subtitle: e.target.value })}
                           />
                           <div className="grid grid-cols-3 gap-6 pt-4">
                             {['Feature One', 'Feature Two', 'Feature Three'].map((feat, idx) => (
                               <div key={idx} className="p-6 bg-white rounded-2xl border border-slate-100 text-center space-y-3 shadow-sm">
                                 <div className="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center mx-auto">
                                   <Sparkles className="w-5 h-5 text-brand" />
                                 </div>
                                 <h4 className="text-sm font-semibold text-slate-900">{feat}</h4>
                                 <p className="text-xs text-slate-400">A short description of this feature and its benefits.</p>
                               </div>
                             ))}
                           </div>
                        </div>
           ...
           <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-6 px-10 py-5 bg-slate-900/90 backdrop-blur-3xl rounded-xl border border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.4)] z-50 animate-in slide-in-from-bottom-10 duration-700">
              <div className="flex items-center gap-3 border-r border-white/10 pr-6">
                 <Cloud className={`w-5 h-5 ${saveStatus === 'saving' ? 'text-amber-400 animate-pulse' : 'text-emerald-400'}`} />
                 <span className="text-xs font-semibold text-white">{saveStatus === 'saving' ? 'Saving...' : 'Auto-Save Active'}</span>
              </div>
      ...
      {showPreview && previewHtml && (
        <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex flex-col animate-in fade-in duration-300">
          <div className="h-16 bg-slate-900 border-b border-white/10 px-8 flex items-center justify-between shrink-0">
            <span className="text-sm font-semibold text-white">Preview Mode</span>
            <div className="flex items-center gap-4">
              <div className="flex bg-slate-800 p-1 rounded-xl">
                <button onClick={() => setViewMode('desktop')} className={`p-2 rounded-lg transition-all ${viewMode === 'desktop' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}><Monitor className="w-4 h-4" /></button>
                <button onClick={() => setViewMode('mobile')} className={`p-2 rounded-lg transition-all ${viewMode === 'mobile' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}><Smartphone className="w-4 h-4" /></button>
              </div>
              <button onClick={() => setShowPreview(false)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all"><X className="w-5 h-5" /></button>
            </div>
          </div>
          <div className="flex-1 flex items-center justify-center p-8 overflow-auto">
            <iframe
              srcDoc={previewHtml}
              className={`bg-white rounded-xl shadow-2xl transition-all duration-500 h-full ${viewMode === 'desktop' ? 'w-full max-w-5xl' : 'w-[400px]'}`}
              title="Site Preview"
            />
          </div>
        </div>
