...
export const NexusInput: React.FC<InputProps> = ({ label, icon: Icon, error, className = '', ...props }) => (
  <div className={`space-y-1.5 ${className || 'w-full'}`}>
    ...
    <div className="relative group">
      ...
      <input
        className={`w-full ${Icon ? 'pl-10' : 'px-3'} py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand/10 focus:border-brand transition-all placeholder:text-slate-400 ${error ? 'border-rose-500' : ''}`}
        {...props}
      />
    </div>
